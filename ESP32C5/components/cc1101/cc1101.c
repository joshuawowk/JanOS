...
bool cc1101_present(cc1101_t *dev) {
    ...
    /* Fast path: known CC110x VERSION values (0x14=CC1101, 0x04=CC1100,
     * 0x17/0x18=common clone revisions). */
    if (v == 0x14 || v == 0x04 || v == 0x17 || v == 0x18) return true;
    /* No chip, or a stuck/floating SPI bus reading all-0 / all-1. */
    if (v == 0x00 || v == 0xFF) return false;
    /* Unlisted VERSION (an off-brand clone): don't reject blindly and don't
     * blindly accept -- a bare VERSION match once false-positived on an nRF24
     * sharing this bus (its STATUS byte ~0x0E read as VERSION). Confirm with a
     * config-register round-trip, which a real CC1101 passes and an nRF24 (whose
     * register map/command set differ) does not. PKTLEN is a safe scratch reg
     * here (restored below; cc1101_config_base() reprograms it right after). */
    uint8_t saved = cc1101_read_reg(dev, CC1101_PKTLEN);
    cc1101_write_reg(dev, CC1101_PKTLEN, 0xA5);
    uint8_t r1 = cc1101_read_reg(dev, CC1101_PKTLEN);
    cc1101_write_reg(dev, CC1101_PKTLEN, 0x5A);
    uint8_t r2 = cc1101_read_reg(dev, CC1101_PKTLEN);
    cc1101_write_reg(dev, CC1101_PKTLEN, saved);
    return (r1 == 0xA5 && r2 == 0x5A);
}
